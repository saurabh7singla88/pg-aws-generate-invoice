// lambda-shopify-orderCancelled.mjs
...
async function moveInvoiceToCancelledFolder(orderName) {
    ...
    try {
        ...
        // Move each matching file to the cancelled folder
...
/**
 * AWS Lambda handler that updates DynamoDB status to Cancelled and moves invoice to cancelled folder.
...
export async function handler(event) {
    try {
        ...
        console.log('Received event payload:', JSON.stringify(payload, null, 2));
        ...
        // Move invoice to cancelled folder in S3
        let movedFiles = [];
        try {
